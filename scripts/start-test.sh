...
docker run -d -v ${homeDir}/stresstest-aws/jmeter-test:/stresstest -v ${homeDir}/scripts/jmeter/network.properties:/stresstest/network.properties \
-v ${homeDir}/scripts/jmeter/host_acct.csv:/stresstest/host_acct.csv --name jmeter  amalrajmani/jmeter:5.2.1 -n -t /stresstest/${jmeter_test_profile}.jmx -q /stresstest/network.properties -j /stresstest/jmeter.log
echo "jmeter test profile ${jmeter_test_profile} started"


echo "start tps monitor..."
docker run -d -v ${homeDir}/scripts/tpsmon:/tpsmon -p 7777:7777 -p 2112:2112 --name tps-monitor amalrajmani/tpsmonitor:v1  --httpendpoint ${quorumEndpoint} --consensus=${consensus} --report /tpsmon/tps-report.csv --prometheusport 2112 --port 7777 --influxdb --influxdb.endpoint "http://host.docker.internal:8086" --influxdb.token "telegraf:test123"
